...
var escape = require('lodash/fp/escape');
...
SpanSchema.prototype.toString = function () {
  var spanElements = this.spans.map(function (span) {
    var newSpan = _objectSpread({}, span, {
...
ParagraphsSchema.prototype.toEscapedPlainText = function () {
  ...
  return escape(this.paragraphs.map(function (p) {
    return p.toPlainText();
  }).join('\n'));
};
...
function parseParagraph(paragraph) {
  ...
  if (paragraph) {
    (function () {
      ...
      for (var i = 0; i < paragraph.childNodes.length; i += 1) {
        _loop(i);
      }
    })();
  }
...
function applyParagraph(spanSchema, text) {
  ...
  if (!spanSchema || !spanSchema.spans.length) {
    return "<p><span style=\"ot:liga;ot:locl,0;\">".concat(text, "</span></p>");
  }
  ...
  if (spanCount === 7) {
    ...
    var wordCount = allWords.length; // If we have 3 words, where the middle word is 1 character

    if (wordCount === 3 && allWords[1].length === 1) {
      newSpans.push.apply(newSpans, _toConsumableArray(fillThreeSpans(spanSchema.spans.slice(0, 3), allWords[0])));
      newSpans.push({
        content: " ".concat(allWords[1], " "),
        style: spanSchema.spans[3].style
      });
      newSpans.push.apply(newSpans, _toConsumableArray(fillThreeSpans(spanSchema.spans.slice(4), allWords[2])));
    } else if (wordCount === 2) {
      newSpans.push.apply(newSpans, _toConsumableArray(fillThreeSpans(spanSchema.spans.slice(0, 3), allWords[0])));
      newSpans.push({
        content: ' ',
      ...
      newSpans.push.apply(newSpans, _toConsumableArray(fillThreeSpans(spanSchema.spans.slice(4), allWords[1])));
    } else if (wordCount === 1) {
      newSpans.push.apply(newSpans, _toConsumableArray(fillThreeSpans(spanSchema.spans.slice(0, 3), allWords[0])));
    ...
    var _allWords = getWords(text);

    var _wordCount = _allWords.length; // handle 3 words with middle initial

    if (_wordCount === 3 && _allWords[1].length === 1) {
      newSpans.push.apply(newSpans, _toConsumableArray(fillTwoSpans(spanSchema.spans.slice(0, 2), false, _allWords[0])));
      newSpans.push({
        content: " ".concat(_allWords[1], " "),
        style: spanSchema.spans[2].style
      });
      newSpans.push.apply(newSpans, _toConsumableArray(fillTwoSpans(spanSchema.spans.slice(3), true, _allWords[2])));
      ...
      newSpans.push.apply(newSpans, _toConsumableArray(fillTwoSpans(spanSchema.spans.slice(3), true, _allWords[1])));
    ...
    var _allWords2 = getWords(text);

    var _wordCount2 = _allWords2.length;

    if (_wordCount2 === 2) {
      newSpans.push.apply(newSpans, _toConsumableArray(fillTwoSpans(spanSchema.spans.slice(0, 2), false, "".concat(_allWords2[0], " "))));
      newSpans.push.apply(newSpans, _toConsumableArray(fillTwoSpans(spanSchema.spans.slice(2), spanSchema.spans[2].length !== 1, _allWords2[1])));
    } else if (_wordCount2 === 1) {
      newSpans.push.apply(newSpans, _toConsumableArray(fillTwoSpans(spanSchema.spans.slice(0, 2), false, _allWords2[0])));
...
function applySpans(paragraphsSchema, text) {
  ...
  var outText = paragraphsSchema.paragraphs.map(function (paragraph, index) {
    ...
    var pText = textParagraphs[index] || '';
  ...
  if (paragraphsSchema.paragraphs.length < textParagraphs.length) {
    ...
    var moreOutText = textParagraphs.slice(paragraphsSchema.paragraphs.length).map(function (textParagraph) {
      return "<p><span style=\"ot:liga;ot:locl,0;\">".concat(textParagraph, "</span></p>");
    });

    (_outText = outText).push.apply(_outText, _toConsumableArray(moreOutText));
  }

  if (outText.length > 1) {
    var found = false;
    outText = outText.reverse().filter(function (t) {
      if (found) {
        return true;
      }
