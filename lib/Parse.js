...
const words = require('lodash/fp/words').convert({
  cap: false
});
  ...
  if (paragraph) {
    ...
    for (let i = 0; i < paragraph.childNodes.length; ++i) {
      ...
      if (styleElement) {
        ...
        const stylesToKeep = []; // eslint-disable-next-line
...
function applyParagraph(spanSchema, text) {
  ...
  if (spanCount === 7) {
    ...
    const wordCount = words.length; // If we have 3 words, where the middle word is 1 character
    ...
    const words = getWords(text);
    const wordCount = words.length; //handle 3 words with middle initial
    ...
    if (wordCount === 2) {
      ...
      newSpans.push(...spanSchema.spans.map((span, index) => {
        return index === 0 ? {
          content: text,
          style: span.style
        } : { ...EMPTY_SPAN
...
function applySpans(paragraphsSchema, text) {
  ...
  let textParagraphs = []; // See if we just have plain text
